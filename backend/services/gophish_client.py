...
def get_api():
    ...
    c = conn.cursor()
    c.execute("SELECT key, value FROM settings WHERE key IN ('sim_gophish_url', 'sim_gophish_api_key')")
    settings = dict(c.fetchall())
    conn.close()

    gophish_url = settings.get('sim_gophish_url', os.getenv("GOPHISH_URL", "http://gophish:3333"))
    gophish_api_key = settings.get('sim_gophish_api_key', os.getenv("GOPHISH_API_KEY", ""))
