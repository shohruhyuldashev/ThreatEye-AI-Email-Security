from fastapi import APIRouter, HTTPException, File, Form, UploadFile, Response, BackgroundTasks, Depends
            ...
            for camp in campaigns:
                ...
                results_summary = {
                    ...
                    "results": {
                        ...
                        "clicked": sum(1 for r in camp.results if r.status in ["Clicked", "Submitted Data"])
                    },
                    "victims": []
                }
                ...
                for r in camp.results:
                    if r.status in ["Clicked", "Submitted Data"]:
...
def get_simulation_history():
    """Returns the history of phishing campaigns from Gophish."""
    from services.gophish_client import get_api
    api = get_api()
    if not api:
        return []
    ...
    try:
        ...
        history = []
        for c in campaigns:
            clicked = sum(1 for r in c.results if r.status in ["Clicked", "Submitted Data"])
            history.append({
                "id": c.id,
                "name": c.name,
                "status": c.status,
...
def trigger_simulation(
    ...
    try:
        ...
        from services.gophish_client import create_campaign_with_generated_email, get_api
        import os
        import re
