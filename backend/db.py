...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        c.execute("INSERT INTO settings (key, value) VALUES ('ai_model', 'llama3')")
...
def _insert_initial_dummy_data(cursor):
    ...
    cursor.execute('''
        INSERT OR IGNORE INTO user_profiles (email, role, behavioral_risk_score, failed_simulations_count, typical_topics)
        VALUES ('user1@example.com', 'Finance', 25, 0, 'Invoices, Payroll, Tax Documents')
    ''')
    cursor.execute('''
        INSERT OR IGNORE INTO user_profiles (email, role, behavioral_risk_score, failed_simulations_count, typical_topics)
        VALUES ('user2@example.com', 'Engineering', 10, 0, 'Code Review, Github, Jira')
    ''')
    cursor.execute('''
        INSERT OR IGNORE INTO user_profiles (email, role, behavioral_risk_score, failed_simulations_count, typical_topics)
        VALUES ('user3@example.com', 'Sales', 45, 1, 'Client Meetings, CRM, Leads')
    ''')
    cursor.execute('''
        INSERT OR IGNORE INTO user_profiles (email, role, behavioral_risk_score, failed_simulations_count, typical_topics)
        VALUES ('user4@example.com', 'HR', 20, 0, 'Recruitment, Policy, Training')
    ''')
    cursor.execute('''
        INSERT OR IGNORE INTO user_profiles (email, role, behavioral_risk_score, failed_simulations_count, typical_topics)
        VALUES ('user5@example.com', 'Executive', 70, 2, 'Strategy, Reports, Confidential')
    ''')
    cursor.execute('''
        INSERT OR IGNORE INTO user_profiles (email, role, behavioral_risk_score, failed_simulations_count, typical_topics)
        VALUES ('user6@example.com', 'IT', 15, 0, 'Networking, Support, Backups')
    ''')
    cursor.execute('''
        INSERT OR IGNORE INTO user_profiles (email, role, behavioral_risk_score, failed_simulations_count, typical_topics)
        VALUES ('user7@example.com', 'Marketing', 30, 0, 'Ads, Social Media, Branding')
    ''')
    cursor.execute('''
        INSERT OR IGNORE INTO user_profiles (email, role, behavioral_risk_score, failed_simulations_count, typical_topics)
        VALUES ('user8@example.com', 'Legal', 50, 1, 'Contracts, Compliance, Litigation')
    ''')
    cursor.execute('''
        INSERT OR IGNORE INTO user_profiles (email, role, behavioral_risk_score, failed_simulations_count, typical_topics)
        VALUES ('user9@example.com', 'Security', 5, 0, 'Logs, Alerts, Incidents')
    ''')
    cursor.execute('''
        INSERT OR IGNORE INTO user_profiles (email, role, behavioral_risk_score, failed_simulations_count, typical_topics)
        VALUES ('user10@example.com', 'Operations', 40, 0, 'Logistics, Supply Chain, Scheduling')
    ''')
